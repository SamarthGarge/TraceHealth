  ...
  return (
    <div className="card animate-fade-up">
      <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
        <div>
          <p className="text-[10px] font-mono tracking-widest text-ink-ghost uppercase mb-0.5">Trend</p>
...
function StatCard({ label, value, sub, accent, delay = 0 }) {
  return (
    ...
    >
      <p className="text-[10px] font-mono tracking-widest text-ink-ghost uppercase mb-3">{label}</p>
      <p className="font-display text-3xl sm:text-4xl font-semibold leading-none" style={{ color: accent ?? "var(--ink)" }}>
        {value ?? "—"}
      </p>
      {sub && <p className="text-xs text-ink-ghost mt-2">{sub}</p>}
    </div>
  );
}
    ...
    <div className="flex items-center justify-between mb-4">
      ...
      {linkTo && (
        <button
          ...
          className="text-xs text-terra hover:text-terra-dark transition-all active:scale-[0.97] flex items-center gap-1"
...
function EmptyState({ icon, label, action, onAction }) {
  return (
    <div className="flex flex-col items-center justify-center py-10 text-center">
      <div className="w-10 h-10 rounded-xl bg-parchment-lo border border-border-soft flex items-center justify-center mb-3">
        {icon}
      </div>
      ...
      <main className="page-main bg-parchment">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
          ...
          <header className="mb-6 sm:mb-8 animate-fade-up">
            <p className="text-[10px] font-mono tracking-widest text-ink-ghost uppercase mb-1">
              Overview
            </p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
            ...
            <div className="lg:col-span-2 space-y-4 sm:space-y-6">
              ...
              <div className="card animate-fade-up" style={{ animationDelay: "200ms" }}>
                  ...
                  {Object.entries(DISEASE_META).map(([key, meta]) => (
                    <button
                      ...
                      className="flex items-center gap-3 px-4 py-3.5 rounded-xl border border-border
                        hover:border-terra hover:bg-terra-dim transition-all text-left group active:scale-[0.98]"
                    ...
                    >
                      <span
                        ...
                        style={{ backgroundColor: meta.dot, transitionTimingFunction: EASE }}
                      />
                      <span className="text-sm font-medium text-ink group-hover:text-terra transition-colors flex-1">
                        {meta.label}
                      </span>
                      <svg
                        className="w-3.5 h-3.5 text-ink-ghost group-hover:text-terra group-hover:translate-x-0.5 ml-auto transition-all"
                        style={{ transitionTimingFunction: EASE }}
                ...
                {!hasConsented && (
                  <p className="mt-4 text-xs text-ink-ghost bg-parchment-lo rounded-xl px-4 py-3 border border-border-soft">
                    You haven't consented to data storage — predictions won't be saved to history.{" "}
                    <button
                      onClick={() => navigate("/profile")}
                      className="text-terra hover:text-terra-dark transition-colors font-medium"
                    >
                      Enable in Profile →
                    </button>
                  </p>
                    ...
                    {recentPredictions.map((p, idx) => {
                      ...
                      return (
                        <div
                          ...
                          className="flex items-center gap-3 sm:gap-4 px-3 sm:px-4 py-3 rounded-xl border border-border
                            hover:border-border-strong hover:bg-parchment-lo/50 transition-all cursor-pointer active:scale-[0.99]"
                          style={{
                        ...
                        >
                          <span className="w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full shrink-0" style={{ backgroundColor: meta.dot ?? "var(--ink-ghost)" }} />
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-ink truncate">{meta.label ?? p.disease}</p>
                            <p className="text-[10px] sm:text-xs text-ink-ghost">{formatDate(p.created_at)}</p>
                          </div>
                          <span
                            className="text-sm font-mono font-semibold shrink-0 hidden sm:block"
                            style={{ color: riskColor }}
                          >
                            {pct}%
                          </span>
                          <span
                            className="px-2 sm:px-2.5 py-0.5 rounded-full text-[10px] font-semibold shrink-0"
                        ...
                        <div key={meta.disease}>
                          ...
                          <div className="grid grid-cols-3 gap-2">
                            {["lr", "rf", "xgb"].map((mKey) => {
                              ...
                              return (
                                <div key={mKey} className="bg-parchment/60 rounded-xl px-3 py-2.5 border border-border-soft">
              ...
              <div className="card animate-fade-up" style={{ animationDelay: "250ms" }}>
                ...
                <div className="space-y-0.5">
                  {[
                    ...
                    { label: "Resources",           to: "/resources",    icon: "M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" },
                    { label: "About Models",        to: "/about-models", icon: "M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" },
                    ...
                    <button
                      ...
                      className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm
                        text-ink-mid hover:text-ink hover:bg-parchment-lo transition-all text-left active:scale-[0.98]"
                      style={{ transitionTimingFunction: EASE, transitionDuration: "200ms" }}
                    >
                      <svg className="w-4 h-4 text-ink-ghost shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.6}>
